...
bitRankW32Int * createBitRankW32Int(uint *bitarray, uint _n, char owner,
    uint _factor) {
...
uint isBitSet(bitRankW32Int * br, uint i) {
  return ((1u << (i % W)) & br->data[i / W] ) >> (i % W);  // agregado el corrimiento de bits luego del &... FIXED by diegocaro
}
...
int save(bitRankW32Int * br, FILE *f) {
  ...
  if (f == NULL)
    return 20;
  if (fwrite(&(n), sizeof(uint), 1, f) != 1)
    return 21;
  if (fwrite(&(br->factor), sizeof(uint), 1, f) != 1)
    return 21;
  if (fwrite(br->data, sizeof(uint), n / W + 1, f) != n / W + 1)
    return 21;
  if (fwrite(br->Rs, sizeof(uint), n / s + 1, f) != n / s + 1)
    return 21;
  return 0;
}
...
int load(bitRankW32Int * br, FILE *f) {
  if (f == NULL)
    return 23;
  if (fread(&(br->n), sizeof(uint), 1, f) != 1)
    return 25;
  br->b = 32;
  uint b = br->b;                      // b is a word
  if (fread(&(br->factor), sizeof(uint), 1, f) != 1)
    return 25;
  br->s = b * br->factor;
  uint s = br->s;
  uint n = br->n;
  ...
  if (!br->data)
    return 1;
  if (fread(br->data, sizeof(uint), br->n / W + 1, f) != n / W + 1)
    return 25;
  br->owner = 1;
  br->Rs = (uint*) malloc(sizeof(uint) * (n / s + 1));
  if (!br->Rs)
    return 1;
  if (fread(br->Rs, sizeof(uint), n / s + 1, f) != n / s + 1)
    return 25;
  return 0;
}
...
uint spaceRequirementInBits(bitRankW32Int * br) {
  return (br->owner ? br->n : 0) + (br->n / br->s) * sizeof(uint) * 8
